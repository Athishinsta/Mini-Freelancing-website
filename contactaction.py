...
cmail = form.getvalue('cmail')
cmessage = form.getvalue('cmessage')

dbcon = pymysql.connect(host="localhost",user="root",password="root", db="freelancing_user_details")
cursor = dbcon.cursor()

if(dbcon):
    query = """insert into Messages(customer_name,customer_email,message) values ("%s","%s","%s") """%(cname,cmail,cmessage)
